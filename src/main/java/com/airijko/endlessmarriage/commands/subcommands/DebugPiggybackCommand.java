...
public class DebugPiggybackCommand extends AbstractPlayerCommand {
    ...
    protected void execute(@Nonnull CommandContext context,
        ...
        if (piggyback.isRiding(senderUuid)) {
            ...
            senderRef.sendMessage(MarriageMessages.debugLine("You hop down off the debug NPC.", "#4fd7f7"));
            return;
        }
        ...
        if (result == PiggybackService.MountResult.SUCCESS) {
            senderRef.sendMessage(MarriageMessages.debugLine("You hop onto the debug NPC.", "#f2a2e8"));
        } else {
            senderRef.sendMessage(MarriageMessages.debugLine("Could not piggyback debug NPC: " + result, "#ff6666"));
        }
    }
}
