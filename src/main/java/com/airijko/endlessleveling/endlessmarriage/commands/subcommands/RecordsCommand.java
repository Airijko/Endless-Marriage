...
public class RecordsCommand extends AbstractPlayerCommand {
    ...
    protected void execute(@Nonnull CommandContext context,
        ...
        if (records.isEmpty()) {
            senderRef.sendMessage(Message.raw(PREFIX + "You have no officiant records.").color(COLOR_INFO));
            return;
        }

        senderRef.sendMessage(Message.raw(PREFIX + "Your officiant records ("
                + records.size() + "):").color(COLOR_INFO));
        ...
        for (OfficiantRecord record : records) {
            ...
            senderRef.sendMessage(Message.raw("  " + typeName + ": "
                    + resolvePlayerName(record.player1()) + " & "
                    + resolvePlayerName(record.player2()) + " (" + date + ")").color(COLOR_INFO));
        }
    }
}
